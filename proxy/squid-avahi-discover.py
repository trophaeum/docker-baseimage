#!/usr/bin/python
#
# use avahi to find a _squid_proxy._tcp provider and return
# a http proxy string suitable for squid
...
def get_proxy_host_port_from_avahi():
    if os.path.isfile("/usr/bin/avahi-browse") == False:
        return None

    service = '_squid_proxy._tcp'

    # Obtain all of the services addresses from avahi, pulling the IPv6
    # addresses to the top.
    addr4 = []
    addr6 = []
    p = Popen(['/usr/bin/avahi-browse', '-kprt', service], stdout=PIPE)
